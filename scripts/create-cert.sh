...
keytool -genkey -noprompt \
			 ...
			 -keystore kafka.$i.keystore.pkcs12 \
			 -keyalg RSA \
			 -storepass confluent \
			 -keypass confluent \
			 -storetype pkcs12
...
keytool -keystore kafka.$i.keystore.pkcs12 -alias $i -certreq -file $i.csr -storepass confluent -keypass confluent -ext "SAN=dns:$i,dns:localhost"
...
keytool -noprompt -keystore kafka.$i.keystore.pkcs12 -alias CARoot -import -file ${CA_PATH}/snakeoil-ca-1.crt -storepass confluent -keypass confluent
#keytool -list -v -keystore kafka.$i.keystore.jks -storepass confluent

# Sign and import the host certificate into the keystore
keytool -noprompt -keystore kafka.$i.keystore.pkcs12 -alias $i -import -file $i-ca1-signed.crt -storepass confluent -keypass confluent -ext "SAN=dns:$i,dns:localhost"
#keytool -list -v -keystore kafka.$i.keystore.jks -storepass confluent

# Create truststore and import the CA cert
keytool -noprompt -keystore kafka.$i.truststore.pkcs12 -alias CARoot -import -file ${CA_PATH}/snakeoil-ca-1.crt -storepass confluent -keypass confluent
...
keytool -export -alias $i -file $i.der -keystore kafka.$i.keystore.pkcs12 -storepass confluent
openssl x509 -inform der -in $i.der -out $i.certificate.pem
keytool -importkeystore -srckeystore kafka.$i.keystore.pkcs12 -destkeystore $i.keystore.p12 -deststoretype PKCS12 -deststorepass confluent -srcstorepass confluent -noprompt
